...
import { editCoupon, getAllCoupons, getShopDetails } from '../../features/shop/ShopSlice';
import { useEffect, useState } from 'react';
import { toast } from 'react-toastify';
import LoadingScreen from '../../components/LoadingScreen';
import AddCouponModal from '../../components/shop/AddCouponModal';

function ShopOwnerCoupon() {
  ...
  const dispatch = useDispatch()
    ...
    const handleModalCoupon = (orderDetail)=>{
      setCouponModal(couponModal ? false:true)
    }

    const handleCouponEdit = (couponDetails)=>{
      dispatch(editCoupon(couponDetails))
      handleModalCoupon()
    }



  const totalRevenue = shopOrders.reduce((acc,order)=>{
 return acc+order.totalBill
  },0)

  useEffect(()=>{
      dispatch(getAllCoupons())
    if(shopError && shopErrorMessage){
      toast.error(shopErrorMessage,{position:'top-left'})
    }
  },[shop,shopError,shopErrorMessage])
...
{
      couponModal &&    <AddCouponModal handleModalCoupon={handleModalCoupon}/>
}
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleModalCoupon} className="flex items-center px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600">
...

{
  shopCoupons.map((coupon)=>{
    return(
                ...
                <td className="px-6 py-4">
                  <span className="text-sm font-medium text-emerald-600">{coupon.couponDiscount}%</span>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-block px-3 py-1 text-xs font-medium ${coupon.isActive?'bg-emerald-100 text-emerald-700':'bg-redq-100 text-red-700'} rounded-full`}>
                    {
...
export default ShopOwnerCoupon;
