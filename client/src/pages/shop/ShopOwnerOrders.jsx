...
import { getAllOrders } from '../../features/shop/ShopSlice';
...
function ShopOwnerOrders() {
 ...
 return acc+order.totalBill
  ...
  const handleModalOrder = (orderDetail)=>{
...
{
      modalOrder &&  <OrderDetailsModal handleModalOrder={handleModalOrder} modalOrder={modalOrder} orderDetail={orderDetail} />
...
return (
                ...
                <td className="px-6 py-4">
                  <span className="font-medium text-slate-900">#{order?._id[0]+order?._id[3]}</span>
                </td>
                <td className="px-6 py-4">
                  <div>
                ...
                <td className="px-6 py-4">
                  <span className="font-medium text-slate-900">₹{order?.totalBill}</span>
                </td>
                <td className="px-6 py-4">
                  <span className={`inline-block px-3 py-1 text-xs font-medium ${order.status==='delivered'? 'bg-emerald-100 text-emerald-700':'bg-red-100 text-red-700'} rounded-full`}>
                    {order.status ==='delivered' ? 'Paid':'Un-Paid'}
                  </span>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <button onClick={() => {handleModalOrder(order)} } className="p-2 hover:bg-slate-100 rounded-lg">
