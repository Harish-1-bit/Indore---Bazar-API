...
const Header = () => {
  ...
  return (
          <header className={location.pathname.includes('/admin') ? 'hidden':"bg-white shadow-sm sticky top-0 z-50"}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            {
              user?<>
              <Link to={'/admin/dashboard'} className="text-gray-700 hover:text-emerald-600 transition-colors">Welcome, {capatlizeWord(user.name)}</Link>
