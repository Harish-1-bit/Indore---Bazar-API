...
import { updateShop } from '../../features/admin/AdminSlice'

const ShopApporoval = ({allShops}) => {

    const approvalList = allShops.filter((shop)=>{
       return shop.status!=='accepted'
    })
    const dispatch = useDispatch()

    const handleShopUpdate = (shopDetails)=>{
        dispatch(updateShop(shopDetails))
    }
            ...
            <div className="space-y-4">
...

                {
                 approvalList.length===0? 
                  <p>No Pending Request for Shop</p>
                 :
